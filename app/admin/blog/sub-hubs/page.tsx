...
import { BlogSubHub } from '@/app/lib/blog-types';

export default function SubHubsListPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('このサブハブを削除してもよろしいですか？関連する投稿も影響を受けます。')) {
      return;
    }

    setDeleting(id);
    try {
      const res = await fetch(`/api/admin/sub-hubs/${id}`, { method: 'DELETE' });
      if (res.ok) {
        setSubHubs(subHubs.filter((sh) => sh.id !== id));
      } else {
        alert('削除に失敗しました');
      }
    } catch (error) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 lg:mb-8">
        <div>
          <h1 className="text-[22px] sm:text-[24px] lg:text-[28px] font-bold text-[#333333]">サブハブ管理</h1>
          <p className="text-[13px] sm:text-[14px] text-[#666666] mt-1">ブログのサブカテゴリーを管理</p>
        </div>
        <Link
          href="/admin/blog/sub-hubs/new"
        ...
        <table className="w-full">
          <thead className="bg-[#f5f6f8]">
            <tr>
              ...
              <th className="text-left text-[13px] font-medium text-[#666666] px-6 py-4">親ハブ</th>
          ...
          <tbody className="divide-y divide-[#e5e5e5]">
            {subHubs.map((subHub) => (
              <tr key={subHub.id} className="hover:bg-[#f5f6f8] transition-colors">
                <td className="px-6 py-4">
                  <div>
                    <Link
                      href={`/admin/blog/sub-hubs/${subHub.id}`}
                      className="text-[14px] font-medium text-[#333333] hover:text-[#1f5bb9]"
                    >
                      {subHub.title}
                    </Link>
                    {subHub.title_en && (
                      <p className="text-[12px] text-[#999999]">{subHub.title_en}</p>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className="text-[13px] text-[#666666] bg-[#f5f6f8] px-2 py-1 rounded">
                    {subHub.hub?.title || '-'}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <code className="text-[13px] text-[#666666] bg-[#f5f6f8] px-2 py-1 rounded">
                    /blog/{subHub.hub?.slug}/{subHub.slug}
                  </code>
                </td>
                <td className="px-6 py-4">
                  <span className={`text-[12px] px-2 py-1 rounded ${
                    subHub.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                  }`}>
                    {subHub.status === 'published' ? '公開' : '下書き'}
                  </span>
                </td>
                <td className="px-6 py-4 text-[13px] text-[#666666]">
                  {formatDate(subHub.created_at)}
                </td>
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      onClick={() => handleDelete(subHub.id)}
                      disabled={deleting === subHub.id}
                      className="text-[13px] text-red-500 hover:underline disabled:opacity-50"
                    >
                      {deleting === subHub.id ? '削除中...' : '削除'}
                    </button>
                  </div>
                </td>
              </tr>
            ...
            {subHubs.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  <p className="text-[#666666]">まだサブハブがありません</p>
                  <Link
                    href="/admin/blog/sub-hubs/new"
